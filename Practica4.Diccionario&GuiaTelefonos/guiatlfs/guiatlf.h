...
#include <string>
using namespace std;
istream & operator>>(istream &is,pair<string,string> &d){ // lee de un fichero las parejas

	  getline(is,d.first,'\t');//lee del dispositivo de entrada(is)
		//parejas de strings nombre tabulador y luego telefono
	  getline(is,d.second);
	  return is;
}
  ...
  */
	  ...
	  public:
		    ...
		    */
		    ...
		    string & operator[](const string &nombre) {//le doy un nombre y me duelve el telefono
			      return datos[nombre];//se indexa a partir de la primera componente
						// y devuelve la segunda (telefono)
		    }


		    string  gettelefono(const string & nombre){
			      map<string,string>::iterator it=datos.find(nombre);
			      if (it==datos.end()) // si no ha encontrado el nombre
					return string(""); //devuelve vacio
			      else return it->second; //devuelve la segunda componente (telefono)
		    }
		    ...
		    */
		    pair<map<string,string>::iterator,bool>  insert(string nombre, string tlf){ // inserta nombre numero telefono(inserccion a mano)
		    ...
		    */
		    pair<map<string,string>::iterator,bool>  insert(pair<string,string> p){
			      ...
			      ret=datos.insert(p); //datos.insert(datos.begin(),p); tambien funcionaría
			      return ret;     //devuelve un pair llamado ret, devuelve la pareja
		    ...
		    */
		    void borrar(const string &nombre){ //localiza el nombre y lo borra
			   ...
			   datos.erase(ret.first,ret.second);
				...
				void borrart(const string &telefono){ //localiza el telefono y lo borra
		    ...
		    */
		     ...
		     */
		    int size()const{//devuelve el tamaño del a guia de telefono
			      return datos.size();
		    }
		    /**
		    ...
		    */
		    Guia_Tlf operator+(const Guia_Tlf & g){ // Une dos guias de telefono
			Guia_Tlf aux(*this);
		    ...
		    */
		    ...
		    friend istream & operator>>(istream & is, Guia_Tlf & g){
			      ...
			      while (is>>p){ // mientras tenga datos (operador de arriba)
				  aux.insert(p); //inserta
			      }
			      g=aux;
			      return is;
		    }
